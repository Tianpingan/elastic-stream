...
class Metadata(Service):
    ...
    def start_cmd(self, node):
        ...
        cmd += "export END_POINT=" + self.pd.get_hostname() + ":12378;"
        cmd += "export KV_END_POINT=" + self.pd.get_hostname() + ":12379;"
        cmd += "export COUNT=" + str(self.count) + ";"
        cmd += "java -cp /opt/*.jar  com.automq.elasticstream.client.tools.e2e.tests.MetadataTest"
        return cmd
    ...
    def kill_node(self, node):
        ...
        wait_until(lambda: not self.alive(node), timeout_sec=5, err_msg="Timed out waiting for Metadata to be killed.")

    def stop_node(self, node):
        idx = self.idx(node)
        self.logger.info("Stopping %s node %d on %s" % (type(self).__name__, idx, node.account.hostname))
        self.signal_node(node)
        wait_until(lambda: not self.alive(node), timeout_sec=5, err_msg="Timed out waiting for Metadata to stop.")
